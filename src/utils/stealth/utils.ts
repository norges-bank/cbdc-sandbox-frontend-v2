...
import { ERC5564_MESSENGER_ADDRESS, getProvider, START_BLOCK } from "../../constants";
import { ERC5564Messenger__factory } from "@symfoni/cbdc-sandbox-contracts-shared";
...
export const signatureToStealthKeys = (signature: string) => {
	...
	const spend = new Wallet(spendingPrivateKey);
	const view = new Wallet(viewingPrivateKey);
	return {
		spend,
		view,
	};
...
export const getAnnoncements = async (fromBlock = 0) => {
	log("getAnnoncements", Math.max(START_BLOCK, fromBlock));
	const messenger = new ERC5564Messenger__factory().attach(ERC5564_MESSENGER_ADDRESS).connect(getProvider());
